...
#define NetworkConfDir "/etc/systemd/network/"
#define eth0NetworkConfDir "/etc/systemd/network/00-bmc-eth0.network"
#define eth1NetworkConfDir "/etc/systemd/network/00-bmc-eth1.network"
#define eth2NetworkConfDir "/etc/systemd/network/00-bmc-eth2.network"
#define eth3NetworkConfDir "/etc/systemd/network/00-bmc-eth3.network"
// Error codes
...
/// Each of the characters in this string separates entry's name from entry's
/// value
#define INI_NAME_VALUE_SEP_CHARS "=:"
/// Each of the characters in this string when found last in input line tells
/// that next line should be considered as current line's continuation
#define INI_MULTILINE_CHARS "\\/"

// Delimiter of values in array
#define INI_ARRAY_DELIMITER ','
// Symbol, opening the segment of array (INI_ARRAY_DELIMITER in segment is not
// considered as delimiter)
#define INI_ARRAY_SEGMENT_OPEN '{'
// Symbol, closing the segment of array
#define INI_ARRAY_SEGMENT_CLOSE '}'
// Escape character in INI file (INI_ARRAY_SEGMENT_OPEN and
// INI_ARRAY_SEGMENT_CLOSE if found inside elements of array and maps should be
// escaped for proper parsing)
...
// Inclusion line
// Please note, that inclusion command must appear in comment section (i.e.
// ;INI_INCLUDE_SEQ)
...
// Calling isspace with signed chars in Visual Studio causes assert failure in
// DEBUG builds This hack is approved by MS itself
#if defined(_MSC_VER)
static inline int __in_isspace(int ch) {
...
static inline std::string &ltrim(std::string &s) {
  ...
  s.erase(s.begin(), std::find_if(s.begin(), s.end(),
                                  [](int ch) { return !__in_isspace(ch); }));
#else
  s.erase(s.begin(),
          std::find_if(s.begin(), s.end(),
                       std::not1(std::ptr_fun<int, int>(__in_isspace))));
#endif
  return s;
}
...
str_replace(std::string &str, const std::string &what, const std::string &rep) {
...
static inline bool path_is_relative(const std::string &path) {
 ...
 * This should be used as a member in reference-counting classes Value, Array
 *and Map
 **/
...
public:
  ...
  T DataObj(const T &defval = T()) const {
    return (_ptr == NULL ? defval : _ptr->val);
  }
 ...
 * This is a simple reference-counting class, storing a pointer to original
 *string It has some functions for easy converting to\from other types Value can
 *contain array (in string representation) and be converted to\from it
 **/
class Value {
public:
  Value() {}
  ...
  // const char* (as any pointer) is defaulting in template to int or bool,
  // which is not what we want
  void Set(const char *value) {
    _val = RefCountPtr<std::string>(std::string(value));
  }
...
public:
  ...
  Array(const std::string &str, char sep = INI_ARRAY_DELIMITER,
  ...
  /// Array is automatically widen (if needed) for that operation to always
  /// succeed
...
public:
  ...
  Map(const std::string &str, char sep = INI_ARRAY_DELIMITER,
      char seg_open = INI_ARRAY_SEGMENT_OPEN,
      char seg_close = INI_ARRAY_SEGMENT_CLOSE,
      char kval = INI_MAP_KEY_VAL_DELIMETER, char esc = INI_ESCAPE_CHARACTER) {
  ...
  void FromString(const std::string &str, char sep = INI_ARRAY_DELIMITER,
    ...
    for (size_t i = 0; i <= str.size(); ++i) {
      if (escaped && i < str.size()) {
        ...
        trim(cur_str);
        std::pair<std::map<Value, Value>::iterator, bool> res = _val->insert(
            std::pair<Value, Value>(Value(cur_key), Value(cur_str)));
        if (!res.second)
  ...
  template <class T, class M> void FromMap(const std::map<T, M> &mp) {
    _val.Copy();
    _val->clear();
    for (typename std::map<T, M>::const_iterator it = mp.begin();
         it != mp.end(); ++it)
      _val->insert(
          std::pair<Value, Value>(Value(it->first), Value(it->second)));
  }
  /// Converts Array to Value
  Value ToValue() const { return Value(ToString()); }
  /// Gets Array from Value
...
public:
  ...
  /// Returns full section name (subsection name will contain '.', separating
  /// subsection part)
  ...
  /// Get comment, associated with this section (written before the line with
  /// the section, or after it)
  std::string Comment() const { return _comment; }
  /// Return all keys in this section
  std::vector<std::string> GetSectionKeys() const {
    std::vector<std::string> keys;
    for (EntryMap::const_iterator it = _entries.begin(); it != _entries.end();
         it++)
...
private:
  ...
  Section(File *file, const std::string &name,
          const std::string &comment = std::string())
  ...
  Section(const Section &cp)
      : _file(cp._file), _name(cp._name), _comment(cp._comment),
        _entries(cp._entries), _comments(cp._comments) {}
...
public:
  ...
  struct PResult {
    ...
    void Set(int code, int line_num = 0,
    ...
    std::string
        file_name; // name of the file with error (will be empty for stream)
...
public:
  ...
  void CopyFrom(const File &lf) {
    for (SectionMap::const_iterator it = lf._sections.begin();
         it != lf._sections.end(); it++) {
...
public:
  ...
  /// Section pointer can be accesed as SectionMap::iterator::second, section
  /// name - as ::first
  ...
  SectionVector FindSubSections(const Section *sect) const {
    ...
    for (SectionMap::const_iterator it = _sections.begin();
      ...
      if (it->second == sect)
        continue;
      if (it->first.find(sect->FullName() + INI_SUBSECTION_DELIMETER) !=
          std::string::npos)
  ...
  /---------------------------------------------------------------------------------------------------------------*/
public:
  /// Load file from input stream, that must be properly opened and prepared for
  /// reading Set @param unload_prev to true for unloading any stuff currently
  /// in memory before loading Set @param rpath to be used as relative path when
  /// searching for inclusions in files
  /// @return 1 if load succeeds, 0 if not
  /// in case load was not succesfull you can access extended information by
  /// calling ParseResult()
  ...
  /// Set @param unload_prev to false for not unloading any stuff currently in
...
protected:
  ...
  /**
   * Parse input line
   * Line must be concatenated (if needed) and trimmed before passing to this
   *function
   ...
   * @param comment - will contain comment in case of return != EMPTY and !=
   *ERROR
  ...
  LineType ParseLine(const std::string &input_line, std::string &section,
    ...
    if (char_is_one_of(input_line.at(0), INI_COMMENT_CHARS)) {
      ...
      // Can't do it that way, cause of 'initialization of non-const reference
      // of type from a temporary of type' stupid GCC rule
  ...
  /// Comments separated from closest section or key-value pair by 1 or more
  /// empty strings would be ignored Default section will be created if needed
  int ParseStream(std::istream &stream, const std::string &def_section,
    ...
    for (int lnc = 1; !stream.eof(); lnc++) {
      ...
      // Handle inclusion
      else if (lt == LEKSYSINI_COMMENT &&
               comment.substr(0, strlen(INI_INCLUDE_SEQ)) == INI_INCLUDE_SEQ) {
      ...
      // Add comment (it can be set with any string type, other than EMPTY and
      // ERROR)
      pcomment += comment;
  ...
  void SaveStream(std::ostream &stream, const SectionMap &pmap) const {
    for (SectionMap::const_iterator it = pmap.begin(); it != pmap.end(); ++it) {
      ...
      if (!it->first.empty())
        stream << INI_SECTION_OPEN_CHARS[0] << it->first
               << INI_SECTION_CLOSE_CHARS[0] << std::endl;
      for (Section::values_iter vit = it->second->ValuesBegin();
           vit != it->second->ValuesEnd(); vit++) {
        stream << vit->first << " " << INI_NAME_VALUE_SEP_CHARS[0] << " "
               << vit->second.AsString();
...
inline Section *Section::FindParent() const {
  return _file->FindParentSection(this);
}
inline Section *Section::GetSubSection(const std::string &name) {
  return _file->GetSubSection(this, name);
}
inline Section *Section::FindSubSection(const std::string &name) const {
  return _file->FindSubSection(this, name);
}
inline SectionVector Section::FindSubSections() const {
  return _file->FindSubSections(this);
}
inline void Section::Save(std::ostream &stream) const {
  return _file->Save(stream, this);
}
} // namespace INI
/*---------------------------------------------------------------------------------------------------------------/
/ Stream operators
